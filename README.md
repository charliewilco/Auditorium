...
The default Codex command uses an ephemeral session, ignores user-level Codex configuration, and disables apps and plugins. This keeps Auditorium-owned runs isolated from unrelated MCP services while retaining the existing Codex authentication in `CODEX_HOME`. Symphony automatically migrates the exact command generated by earlier releases; other explicit `codex.command` values remain custom overrides for repositories that intentionally need different Codex configuration or MCP services.
