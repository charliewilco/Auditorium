...
The default Codex command uses an ephemeral session, ignores user-level Codex configuration, and disables apps and plugins. This keeps Auditorium-owned runs isolated from unrelated MCP services while retaining the existing Codex authentication in `CODEX_HOME`. Set `codex.command` explicitly in `WORKFLOW.md` when a repository intentionally needs different Codex configuration or MCP services.
